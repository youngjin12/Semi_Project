   		   			...
   		   			for(var i  in list){
   						     ...
   						     '</table>'+
   						   	 ...
   						   	 '<form>'+
   						     	 ...
   						     	 '<input type="hidden" id="pZero" value="'+list[0].pName+'">'+	
   						   		'<input type="hidden" id="pSum" value="'+list.size+'">'+	
   						     '</form>' <%-- 본문 끝--%>
	...
	<script>
		...
		$("#check_module").click(function () {
			...
			IMP.request_pay({
					...
					name: '주문명: ' + $('#pZero').val() + '외 ' + $('#pSize').val() + '개',
					//결제창에서 보여질 이름
...
<script>
  function pageChange(rsp){
	...
	//location.href="<%=request.getContextPath()%>/paymentResult.do";
	}
